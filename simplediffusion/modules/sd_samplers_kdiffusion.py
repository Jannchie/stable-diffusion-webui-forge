...
from simplediffusion.modules import sd_samplers_common, sd_samplers_cfg_denoiser
...
class KDiffusionSampler(sd_samplers_common.Sampler):
    ...
    def get_sigmas(self, p, steps):
        ...
        if p.sampler_noise_scheduler_override:
            ...
            if opts.sigma_min not in [0, m_sigma_min]:
    ...
    def sample_img2img(
        ...
        xi = self.model_wrap.predictor.noise_scaling(
            sigma_sched[0], noise, x, max_denoise=False
        )
    ...
    def sample(
        ...
        x = self.model_wrap.predictor.noise_scaling(
            sigmas[0], x, torch.zeros_like(x), max_denoise=opts.sgm_noise_multiplier
        )
