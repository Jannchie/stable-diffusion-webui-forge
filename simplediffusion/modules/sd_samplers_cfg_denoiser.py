...
class CFGDenoiser(torch.nn.Module):
    ...
    def combine_denoised(
        self, x_out, conds_list, uncond, cond_scale, timestep, x_in, cond
    ):
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, s_min_uncond, image_cond):
        ...
        if self.classic_ddim_eps_estimation:
            ...
            real_sigma = fake_sigmas[
                sigma.round().long().clip(0, int(fake_sigmas.shape[0]))
            ]
            real_sigma_data = 1.0
            x = x * (
                ((real_sigma**2.0 + real_sigma_data**2.0) ** 0.5)[:, None, None, None]
            )
            sigma = real_sigma
        ...
        uncond = (
            prompt_parser.reconstruct_cond_batch(uncond, self.step)
        ...
        if self.mask is not None:
            ...
            noisy_initial_latent = predictor.noise_scaling(
                sigma[:, None, None, None],
                torch.randn_like(self.init_latent).to(self.init_latent),
                self.init_latent,
                max_denoise=False,
            )
        ...
        denoiser_params = CFGDenoiserParams(
            x, image_cond, sigma, self.step, self.steps, cond, uncond, self
        )
        ...
        denoised, cond_pred, uncond_pred = sampling_function(
            self,
        ...
        store_latent(preview)

        after_cfg_callback_params = AfterCFGCallbackParams(
            denoised, self.step, self.steps
        )
