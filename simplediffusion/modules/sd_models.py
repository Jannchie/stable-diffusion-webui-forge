...
def match_checkpoint_to_name(name):
    ...
    for ckptname in checkpoints_list.values():
        ...
        if (name in title) or (title in name):
            return (
                ckptname.short_title
                if shared.opts.sd_checkpoint_dropdown_use_short
                else ckptname.name.split(" [")[0]
            )
...
def load_model(checkpoint_info):
    ...
    additional_state_dicts = model_data.forge_loading_parameters.get(
        "additional_modules", []
    )

    timer.record("cache state dict")

    dynamic_args["forge_unet_storage_dtype"] = model_data.forge_loading_parameters.get(
...
def apply_token_merging(sd_model, token_merging_ratio):
    ...
    sd_model.forge_objects.unet = TomePatcher().patch(
        model=sd_model.forge_objects.unet, ratio=token_merging_ratio
    )
