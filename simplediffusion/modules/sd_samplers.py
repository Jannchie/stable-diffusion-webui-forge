...
from simplediffusion.modules import sd_samplers_common, sd_samplers_kdiffusion
...
def find_sampler_config(name):
    return all_samplers_map.get(name) if name is not None else all_samplers[0]
...
def create_sampler(name, model):
    ...
    if model.is_sdxl and config.options.get("no_sdxl", False):
        raise RuntimeError(f"Sampler {config.name} is not supported for SDXL")
