...
import backend.memory_management
import backend.patcher.clipvision
from backend.patcher.base import ModelPatcher
from modules.modelloader import load_file_from_url
from modules_forge.shared import add_supported_preprocessor, preprocessor_dir
from simplediffusion.utils import numpy_to_pytorch, resize_image_with_pad
...
class Preprocessor:
    ...
    def setup_model_patcher(
        ...
        **kwargs
        ...
        if load_device is None:
            load_device = backend.memory_management.get_torch_device()

        if offload_device is None:
            offload_device = torch.device("cpu")

        if not backend.memory_management.should_use_fp16(load_device):
            dtype = torch.float32
    ...
    def move_all_model_patchers_to_gpu(self):
        backend.memory_management.load_models_gpu([self.model_patcher])
        return

    def send_tensor_to_model_device(self, x):
        return x.to(
            device=self.model_patcher.current_device, dtype=self.model_patcher.dtype
        )
...
class PreprocessorClipVision(Preprocessor):
    ...
    def load_clipvision(self):
        ...
        if ckpt_path in PreprocessorClipVision.global_cache:
            ...
            self.clipvision = backend.patcher.clipvision.load(ckpt_path)
