...
from simplediffusion.modules.sd_samplers_common import (
    Sampler,
...
class StableDiffusionProcessing:
    ...
    comments: dict = None
    sampler: Sampler | None = field(default=None, init=False)
...
class Processed:
    def __init__(
        ...
        self.seed = (
            int(self.seed[0] if isinstance(self.seed, list) else self.seed)
            if self.seed is not None
            else -1
        )
        self.subseed = (
            int(self.subseed[0] if isinstance(self.subseed, list) else self.subseed)
...
def create_infotext(
    ...
    if uses_ensd:
        uses_ensd = is_sampler_using_eta_noise_seed_delta(p)
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    with torch.inference_mode():
        ...
        for n in range(p.n_iter):
            ...
            if hasattr(p.sd_model, "alphas_cumprod") and hasattr(
                p.sd_model, "alphas_cumprod_original"
            ):
                p.sd_model.alphas_cumprod = p.sd_model.alphas_cumprod_original.to(
                    shared.device
                )
                ...
                if opts.use_downcasted_alpha_bar:
                    ...
                    p.sd_model.alphas_cumprod = p.sd_model.alphas_cumprod.half().to(
                        shared.device
                    )
                if opts.sd_noise_schedule == "Zero Terminal SNR":
                    p.extra_generation_params["Noise Schedule"] = opts.sd_noise_schedule
                    p.sd_model.alphas_cumprod = rescale_zero_terminal_snr_abar(
                        p.sd_model.alphas_cumprod
                    ).to(shared.device)

            alphas_cumprod_modifiers = p.sd_model.forge_objects.unet.model_options.get(
                "alphas_cumprod_modifiers", []
            )
            ...
            for hook in p.hooks:
                ...
                for hook in p.hooks:
                    hook.postprocess_batch_list(p, batch_params, batch_number=n)
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def calculate_target_resolution(self):
        ...
        if self.hr_resize_x == 0 and self.hr_resize_y == 0:
            ...
            if self.hr_resize_y == 0:
                ...
                self.calculate_hr_size()

    def calculate_hr_size(self):
    ...
    def init(self):
        ...
        if (
            ...
            raise RuntimeError(f"could not find upscaler named {self.hr_upscaler}")
    ...
    def sample(
        ...
        if self.firstpass_image is not None and self.enable_hr:
            ...
            for hook in self.hooks:
                hook.process_before_every_sampling(
    ...
    def calculate_hr_conds(self):
        ...
        with devices.autocast():
            self.hr_uc = prompt_parser.get_learned_conditioning(
                hr_negative_prompts, self.firstpass_steps, total_steps
            )
            self.hr_c = prompt_parser.get_multicond_learned_conditioning(
                hr_prompts, self.firstpass_steps, total_steps
            )
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
        ...
        if self.inpainting_fill == 2:
            self.init_latent = (
                ...
                + create_random_tensors(
                    self.init_latent.shape[1:], all_seeds[: self.init_latent.shape[0]]
                )
                * self.nmask
            )
    ...
    def sample(
        ...
        for hook in self.hooks:
            hook.process_before_every_sampling(self)
