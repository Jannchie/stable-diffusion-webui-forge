...
from simplediffusion.utils import numpy_to_pytorch, resize_image
...
class ControlNetHook(Hook):
    sorting_priority = 10

    def __init__(self, units: list[ControlNetUnit] | None = None):
        super().__init__()
        if units is None:
            units = []
        self.units: list[ControlNetUnit] = units

    def get_enabled_units(self) -> list[ControlNetUnit | dict]:
        ...
        return [x for x in units if x.enabled]
    ...
    def try_crop_image_with_a1111_mask(
        ...
        if (
            ...
            input_image = [
                resize_image(resize_mode.int_value(), i, mask.width, mask.height)
    ...
    def get_input_data(
        ...
        h: int,
        w: int,
    ) -> Tuple[list, external_code.ResizeMode]:
        ...
        if unit.input_mode == external_code.InputMode.MERGE:
            ...
            image = self.try_crop_image_with_a1111_mask(
                p, unit.preprocessor, image, resize_mode
            )
            ...
            if mask is not None:
                ...
                mask = self.try_crop_image_with_a1111_mask(
                    p, unit.preprocessor, mask, resize_mode
                )
        ...
        if (
            ...
            and unit.preprocessor.expand_mask_when_resize_and_fill
        ):
            new_image_list = []
    ...
    def process_unit(
        ...
        **kwargs,
        ...
        input_list, resize_mode = self.get_input_data(p, unit, h, w)
        ...
        for input_image, input_mask in optional_tqdm(input_list, len(input_list) > 1):
            ...
            # cv2.imwrite("./controlnet_condition.png", preprocessor_output)
        ...
        if control_masks:
            ...
            for input_mask in control_masks:
                fill_border = unit.preprocessor.fill_mask_with_one_when_resize_and_fill
    ...
    def bound_check_params(self, unit: ControlNetUnit) -> None:
        """
        ...
        if unit.processor_res < 0:
            unit.processor_res = int(
                unit.preprocessor.slider_resolution.gradio_update_kwargs.get(
                    "value", 512
                )
            )
        ...
        if unit.threshold_a < 0:
            unit.threshold_a = int(
                unit.preprocessor.slider_1.gradio_update_kwargs.get("value", 1.0)
            )

        if unit.threshold_b < 0:
            unit.threshold_b = int(
                unit.preprocessor.slider_2.gradio_update_kwargs.get("value", 1.0)
            )
    ...
    def process(self, p: StableDiffusionProcessing):
        ...
        for i, unit in enumerate(enabled_units):
            print(f"Processing ControlNet Unit {i + 1}/{len(enabled_units)}")
            self.bound_check_params(unit)
            params = ControlNetCachedParameters()
            self.process_unit(p, unit, params)
    ...
    def postprocess_batch_list(self, p, pp, *args, **kwargs):
        print("Postprocessing batch list")
        for i, unit in enumerate(self.get_enabled_units()):
...
#     shared.opts.add_option("controlnet_disable_photopea_edit", shared.OptionInfo(False, "Disable photopea edit", gr.Checkbox, {"interactive": True}, section=section))
#     shared.opts.add_option("controlnet_photopea_warning", shared.OptionInfo(True, "Photopea popup warning", gr.Checkbox, {"interactive": True}, section=section))
